...
> = {
  blue: {
    ...
    icon: 'bg-[#1e6bb8]',
    border:
      'border-0 border-l-[3px] border-b-[3px] border-l-[#3d8fd4] border-b-[#3d8fd4] hover:border-l-[#6bb0ef] hover:border-b-[#6bb0ef]',
  },
  green: {
    card: 'bg-[#1f2e10]/55',
    ...
    icon: 'bg-[#5a7a2e]',
    border:
      'border-0 border-l-[3px] border-b-[3px] border-l-[#7a9a3a] border-b-[#7a9a3a] hover:border-l-[#9cbc55] hover:border-b-[#9cbc55]',
  },
}
    ...
    <div className={`relative z-10 w-full ${className}`.trim()}>
      <div className="mx-auto grid w-full max-w-7xl grid-cols-1 gap-4 px-4 pb-5 pt-2 sm:gap-5 sm:px-6 sm:pb-6 md:grid-cols-3 lg:gap-6 lg:px-10 lg:pb-7">
        {FEATURES.map((card) => {
          ...
          return (
            <div key={card.title} className={`hero-feature-shell hero-feature-shell--${card.tint}`}>
              <Link
                href={card.href}
