...
export type VeteranApplicationFormProps = {
  /** `page` = full Next.js page chrome; `embed` = form only (WordPress template supplies header). */
  layout?: 'page' | 'embed'
  submitUrl?: string
  missionUrl?: string
...
export default function VeteranApplicationForm({
  layout = 'page',
  submitUrl = '/api/veteran-application',
  missionUrl = '/mission',
  successStoriesUrl = '/success-stories',
}: VeteranApplicationFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const wpCfg = (typeof window !== 'undefined'
        ? (window as Window & {
            cvcVeteranApplication?: { nonce?: string; ajaxAction?: string }
          }).cvcVeteranApplication
        : undefined) as { nonce?: string; ajaxAction?: string } | undefined

      if (submitUrl.includes('admin-ajax.php') && wpCfg?.ajaxAction) {
        fd.append('action', wpCfg.ajaxAction)
        if (wpCfg.nonce) {
          fd.append('nonce', wpCfg.nonce)
        }
      }
  ...
  const Wrapper = isEmbed ? 'div' : 'main'
  const wrapperClassName = isEmbed
    ? 'cvc-veteran-application-form text-cvc-fg'
    : 'min-h-screen bg-cvc-page pt-[var(--cvc-nav-height)] text-cvc-fg'

  return (
    <Wrapper className={wrapperClassName}>
      <section className={isEmbed ? 'py-0' : 'py-16'}>
              ...
              <div className="space-y-6">
                ...
                <div className="border-t border-cvc-border pt-6">
                  ...
                  <ul className="space-y-2">
                    <li>
                      <a href={missionUrl} className="font-medium text-sky-600 underline hover:text-sky-500 dark:text-sky-400 dark:hover:text-sky-300">
                        For more details check out our MISSION page
                      </a>
                    </li>
                    <li>
                      <a href={successStoriesUrl} className="font-medium text-sky-600 underline hover:text-sky-500 dark:text-sky-400 dark:hover:text-sky-300">
