...
/**
 * Hand-heart icon for the nav donate button.
 */
function cvc_donate_nav_icon() {
	return '<svg class="cvc-nav__donate-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M11 14h2a2 2 0 1 0 0-4h-3c-.6 0-1.1.2-1.4.6L3 16"/><path stroke-linecap="round" stroke-linejoin="round" d="m7 20 1.6-1.4c.3-.4.8-.6 1.4-.6h4c1.1 0 2.1-.4 2.8-1.2l4.6-4.4a2 2 0 0 0-2.75-2.91l-4.2 3.9"/><path stroke-linecap="round" stroke-linejoin="round" d="m2 15 6 6"/><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.5c.7-.7 1.5-1.6 1.5-2.7A2.73 2.73 0 0 0 16 4a2.78 2.78 0 0 0-5 1.8c0 1.2.8 2 1.5 2.8L16 12Z"/></svg>';
}
...
/**
 * Sun/moon icons for the circular theme toggle.
 */
function cvc_theme_toggle_icon( $variant ) {
	$class = 'cvc-theme-toggle__icon';

	if ( 'sun' === $variant ) {
		return '<svg class="' . esc_attr( $class ) . ' cvc-theme-toggle__icon--sun" width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"/></svg>';
	}

	return '<svg class="' . esc_attr( $class ) . ' cvc-theme-toggle__icon--moon" width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"/></svg>';
}

/**
 * Markup for the circular theme toggle button.
 */
function cvc_theme_toggle_button( $extra_class = '', $button_id = '' ) {
	$id_attr = $button_id ? ' id="' . esc_attr( $button_id ) . '"' : '';
	$class   = trim( 'cvc-theme-toggle ' . $extra_class );

	return '<button type="button" class="' . esc_attr( $class ) . '"' . $id_attr . ' aria-label="' . esc_attr__( 'Toggle light/dark theme', 'cvc-theme' ) . '">' .
		cvc_theme_toggle_icon( 'sun' ) .
		cvc_theme_toggle_icon( 'moon' ) .
		'</button>';
}
